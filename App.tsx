    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100 overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 lg:py-10">
        ...
        {view === 'dashboard' ? (
          ...
          /* CLIENTS VIEW */
          <div className="animate-fadeIn pb-32 space-y-8 sm:space-y-12 px-4 sm:px-6">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex items-center gap-4 sm:gap-6">
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-white rounded-xl sm:rounded-2xl flex items-center justify-center text-rose-500 border border-slate-100 shadow-sm shrink-0">
                  <Users size={24} className="sm:size-[32px]" />
                </div>
                <div className="min-w-0">
                  <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold uppercase tracking-tighter text-slate-800 truncate">Meus Clientes</h2>
                  <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 sm:mt-2">{clientes.length} Clientes Cadastrados</p>
                </div>
              </div>
              <button onClick={() => setModalClientes(true)} className="py-3 px-6 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg hover:shadow-indigo-500/20 transition-all flex items-center gap-2" style={{ backgroundColor: appColor }}>
                <Plus size={18} /> Novo Cliente
              </button>
            </header>

            <section className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm overflow-hidden animate-scaleUp">
              <div className="p-6 border-b border-slate-50 flex items-center gap-4 bg-slate-50/50">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full bg-white px-5 py-3 rounded-xl border border-slate-200 text-sm font-medium outline-none focus:ring-2 focus:ring-rose-500/20 transition-all"
                  />
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50/50">
                      <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">Cliente</th>
                      <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">Contato</th>
                      <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">Ações</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {clientes.filter(c =>
                      c.nome.toLowerCase().includes(filtroClientes.toLowerCase()) ||
                      (c.telefone || '').includes(filtroClientes) ||
                      (c.email || '').toLowerCase().includes(filtroClientes.toLowerCase())
                    ).length > 0 ? clientes.filter(c =>
                      c.nome.toLowerCase().includes(filtroClientes.toLowerCase()) ||
                      (c.telefone || '').includes(filtroClientes) ||
                      (c.email || '').toLowerCase().includes(filtroClientes.toLowerCase())
                    ).map(c => (
                      <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 font-bold uppercase text-xs group-hover:bg-white group-hover:shadow-sm transition-all">
                              {c.nome.charAt(0)}
                            </div>
                            <div>
                              <p className="text-sm font-extrabold text-slate-800 uppercase">{c.nome}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <div className="space-y-1">
                            {c.telefone && <p className="text-xs font-bold text-slate-500 flex items-center gap-2"><div className="w-1 h-1 rounded-full bg-emerald-400" /> {c.telefone}</p>}
                            {c.email && <p className="text-[10px] font-medium text-slate-400 flex items-center gap-2"><div className="w-1 h-1 rounded-full bg-indigo-400" /> {c.email}</p>}
                          </div>
                        </td>
                        <td className="px-6 py-5 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button className="p-2 text-slate-300 hover:text-indigo-500 hover:bg-indigo-50 rounded-lg transition-all"><Settings size={16} /></button>
                            <button onClick={() => setClientes(prev => prev.filter(x => x.id !== c.id))} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    )) : (
                      <tr>
                        <td colSpan={3} className="px-6 py-24 text-center">
                          <div className="flex flex-col items-center justify-center max-w-xs mx-auto">
                            ...
                            <p className="text-base font-black text-slate-800 uppercase tracking-tight">Nenhum cliente!</p>
                            <p className="text-[11px] text-slate-400 font-bold uppercase mt-2 tracking-widest leading-relaxed">Sua lista está vazia. Comece a cadastrar para gerenciar melhor seus atendimentos.</p>
                          </div>
                        </td>
                      </tr>
                    )}

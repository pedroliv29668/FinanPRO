...
const meses = ['JANEIRO', 'FEVEREIRO', 'MAR├çO', 'ABRIL', 'MAIO', 'JUNHO', 'JULHO', 'AGOSTO', 'SETEMBRO', 'OUTUBRO', 'NOVEMBRO', 'DEZEMBRO'];
...
const App: React.FC = () => {
  ...
  const [appName, setAppName] = useState(() => getSaved('appName', 'Gest├úo Cl├¡nica Est├®tica'));
  ...
  const [gastosFixos, setGastosFixos] = useState<GastoFixo[]>(() => getSaved('gastosFixos', [
    ...
    { id: '2', nome: '├üGUA', valor: 0, isPadrao: true },
    { id: '3', nome: 'LUZ', valor: 0, isPadrao: true },
    { id: '4', nome: 'INTERNET', valor: 0, isPadrao: true },
    { id: '5', nome: 'SAL├üRIOS FIXOS', valor: 0, isPadrao: true },
  ...
  const [formReceita, setFormReceita] = useState({
    ...
    cliente: '',
    procedimento: '',
    formaPagamento: 'Pix' as 'Pix' | 'Cart├úo' | 'Dinheiro',
    valor: ''
  });
    ...
    if (isAuthenticated) {
      const loadData = async () => {
        try {
          ...
          const name = user.user_metadata?.full_name || user.email?.split('@')[0] || 'Usu├írio';
          setUserName(name);
          ...
          console.error('Falha cr├¡tica no carregamento:', err);
  ...
  const pagamentosResumo = useMemo(() => {
    return receitasMes.reduce((acc, curr) => {
      ...
      return acc;
    }, { Pix: 0, Cart├úo: 0, Dinheiro: 0 } as Record<string, number>);
  }, [receitasMes]);
  ...
  const analyticsData = useMemo(() => {
    ...
    const proporcaoGastos = [
      ...
      { name: 'Vari├íveis', value: totalVariaveis, color: '#f43f5e' }
  ...
  useEffect(() => {
    // 1. Sempre salva no LocalStorage (mais r├ípido, sem rede)
    ...
    // 2. S├│ sincroniza com Supabase se:
    //    - Estiver autenticado
  ...
  const handleAddAgendamento = (ag: Omit<Agendamento, 'id'>) => {
    ...
    // Se houver forma de pagamento selecionada, lan├ºa como receita
  ...
  const handleAddReceita = (e: React.FormEvent) => {
    ...
    setReceitas((prev) => [{
      id: Date.now(), valor: v, procedimento: formReceita.procedimento || 'Servi├ºo',
      ...
      categoria: 'Entrada', descricao: 'Lan├ºamento Manual'
    }, ...prev]);
  ...
  const handleAddDespesa = (e: React.FormEvent) => {
    ...
    setDespesasVariaveis((prev) => [{
      ...
      categoria: 'Sa├¡da', data: formDespesa.data, mes: dObj.getUTCMonth(), ano: dObj.getUTCFullYear(),
      formaPagamento: formDespesa.formaPagamento
    }, ...prev]);
    setFormDespesa({ ...formDespesa, descricao: '', valor: '' });
  };

  // L├│gica de Lembretes de Amanh├ú
  ...
  const handleGenerateInsight = async () => {
    ...
    const summary = {
      ...
      proje├º├úoCrescimento: projecaoSelecionada
    };
    ...
    if (agendamentosAmanha.length > 0 && 'Notification' in window) {
      if (Notification.permission === 'default') {
        ...
        if (lastNotify !== todayStr) {
          new Notification("­ƒÆà Lembrete de Agenda", {
            body: `Amanh├ú voc├¬ tem ${agendamentosAmanha.length} atendimentos agendados!`,
          });
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><Activity className="animate-spin text-slate-400" size={40} /></div>;

  if (!isAuthenticated) {
    return <AuthScreen appName={appName} appColor={appColor} />;
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 lg:py-10">
        <header className="flex justify-between items-center mb-6 sm:mb-8 bg-white/70 backdrop-blur-md sticky top-4 z-[100] px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl border border-white shadow-sm">
          ...
          <div className="hidden sm:flex items-center gap-2 px-4 py-1.5 bg-slate-50 rounded-full border border-slate-100">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{view === 'dashboard' ? 'Overview' : view === 'agenda' ? 'Agenda Semanal' : view === 'clientes' ? 'Clientes' : view === 'marketing' ? 'Marketing' : 'Relat├│rios'}</span>
          </div>
          ...
          <main className="animate-fadeIn space-y-8 sm:space-y-10 lg:space-y-14">
              ...
              <div className="bg-slate-900 p-5 rounded-2xl sm:rounded-3xl flex items-center justify-between text-white shadow-xl border border-white/10 overflow-hidden relative group transition-all hover:scale-[1.01]">
                ...
                <div className="flex items-center gap-4 sm:gap-6 relative z-10">
                  ...
                  <div>
                    ...
                    <p className="text-sm sm:text-lg font-extrabold tracking-tight">Voc├¬ tem {agendamentosAmanha.length} atendimentos para amanh├ú!</p>
                  </div>
                </div>
                <button onClick={() => setView('agenda')} className="px-4 py-2 bg-indigo-500 hover:bg-indigo-400 text-white rounded-xl text-[10px] sm:text-xs font-bold uppercase tracking-widest transition-all relative z-10 shadow-lg">Ver Agenda</button>
              </div>
            )}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
                  ...
                  >
                    ...
                    <div className="relative flex items-center gap-3">
                      ...
                      <span className="relative z-10">Consultoria Estrat├®gica</span>
                    </div>
                  </button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
              <div className="lg:col-span-8 bg-slate-900 rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-8 text-white shadow-xl relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 relative z-10">
                  ...
                  <div className="p-3 sm:p-4 bg-white/5 rounded-xl sm:rounded-2xl border border-white/5">
                    <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase mb-1">Sa├¡das Totais</p>
                  ...
                  <div className="p-3 sm:p-4 bg-white/5 rounded-xl sm:rounded-2xl border border-white/5">
                    <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase mb-1">M├¬s Anterior</p>
                    <p className="text-lg sm:text-xl font-extrabold text-slate-400">{formatMoeda(totalReceitasAnterior)}</p>
                    <p className="mt-2 text-[8px] sm:text-[9px] text-indigo-400 font-bold">PROJE├ç├âO: +{projecaoSelecionada}%</p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8" id="section-lancamentos">
              <section className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 lg:p-12 border border-slate-100 shadow-md">
                <h3 className="text-base sm:text-lg font-extrabold text-slate-800 uppercase mb-6 sm:mb-8 flex items-center gap-3"><Plus size={20} className="text-emerald-500 sm:size-[24px]" /> Lan├ºar Receita</h3>
                <form onSubmit={handleAddReceita} className="space-y-4 sm:space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="text-[9px] font-bold uppercase text-slate-400 ml-1">Pagamento</label><div className="flex gap-1 p-1 bg-slate-100 rounded-xl">{['Pix', 'Cart├úo', 'Dinheiro'].map(f => <button key={f} type="button" onClick={() => setFormReceita({ ...formReceita, formaPagamento: f as any })} className={`flex-1 py-1.5 sm:py-2 rounded-lg text-[8px] sm:text-[9px] font-bold uppercase transition-all ${formReceita.formaPagamento === f ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400'}`}>{f}</button>)}</div></div>
              ...
              <section className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 lg:p-12 border border-slate-100 shadow-md">
                <h3 className="text-base sm:text-lg font-extrabold text-slate-800 uppercase mb-6 sm:mb-8 flex items-center gap-3"><TrendingDown size={20} className="text-rose-500 sm:size-[24px]" /> Lan├ºar Despesa</h3>
                <form onSubmit={handleAddDespesa} className="space-y-4 sm:space-y-5">
                  ...
                  <div><label className="text-[9px] font-bold uppercase text-slate-400 ml-1">Descri├º├úo</label><input type="text" placeholder="O que foi pago?" value={formDespesa.descricao} onChange={e => setFormDespesa({ ...formDespesa, descricao: e.target.value })} className="w-full bg-slate-50 p-3 sm:p-3.5 rounded-xl border border-slate-200 text-sm font-bold focus:ring-2 focus:ring-rose-50 outline-none uppercase" required /></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="text-[9px] font-bold uppercase text-slate-400 ml-1">Pagamento</label><div className="flex gap-1 p-1 bg-slate-100 rounded-xl">{['Pix', 'Cart├úo', 'Dinheiro'].map(f => <button key={f} type="button" onClick={() => setFormDespesa({ ...formDespesa, formaPagamento: f as any })} className={`flex-1 py-1.5 sm:py-2 rounded-lg text-[8px] sm:text-[9px] font-bold uppercase transition-all ${formDespesa.formaPagamento === f ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-400'}`}>{f}</button>)}</div></div>
                  ...
                  <button type="submit" className="w-full bg-rose-500 text-white py-3.5 sm:py-4 rounded-xl font-bold uppercase text-[9px] sm:text-[10px] tracking-widest shadow-lg hover:bg-rose-600 transition-all active:scale-[0.98] flex items-center justify-center gap-3 mt-2 sm:mt-4"><Save size={18} /> Salvar Sa├¡da</button>
                </form>
              </section>
            </div>
            ...
            <section className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 lg:p-12 border border-slate-100 shadow-sm animate-fadeIn">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10">
                ...
                <div className="space-y-4">
                  <p className="text-[9px] sm:text-[10px] font-bold text-rose-500 uppercase tracking-widest border-b border-rose-100 pb-3 sm:pb-4">Detalhamento de Sa├¡das</p>
                  <div className="space-y-3 max-h-[360px] sm:max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                    {despesasMes.map(d => {
                    ...
                    {despesasMes.length === 0 && <p className="text-center py-10 text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Nenhuma sa├¡da registrada</p>}
                  </div>
                </div>
              </div>
            </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                {lucroReal > 0 && (
                  <div className="p-5 sm:p-6 bg-emerald-50 rounded-xl sm:rounded-2xl border border-emerald-100 hover:shadow-md transition-all">
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 text-emerald-600">
                      ...
                      <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">Sa├║de Financeira</p>
                    </div>
                    <p className="text-xs sm:text-sm font-medium text-slate-700 leading-relaxed">
                      Seu neg├│cio est├í operando com lucro. Considere reinvestir cerca de <span className="font-bold text-emerald-600">{formatMoeda(lucroReal * 0.2)}</span> (20%) em marketing para atrair mais clientes VIP.
                    </p>
                  </div>
                )}
                {totalReceitas > 0 && (
                  <div className="p-5 sm:p-6 bg-blue-50 rounded-xl sm:rounded-2xl border border-blue-100 hover:shadow-md transition-all">
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 text-blue-600">
                      ...
                      <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">Proje├º├úo de Faturamento</p>
                    </div>
                    <p className="text-xs sm:text-sm font-medium text-slate-700 leading-relaxed mb-3">
                      Com base no crescimento de <span className="font-bold text-blue-600">+{projecaoSelecionada}%</span>, a expectativa para os pr├│ximos meses ├®:
                    </p>
                ...
                {topServicos.length > 0 && (
                  <div className="p-5 sm:p-6 bg-indigo-50 rounded-xl sm:rounded-2xl border border-indigo-100 hover:shadow-md transition-all">
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 text-indigo-600">
                      ...
                      <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">Estrat├®gia de Produto</p>
                    </div>
                    <p className="text-xs sm:text-sm font-medium text-slate-700 leading-relaxed">
                      O procedimento <span className="font-bold text-indigo-600">"{topServicos[0][0]}"</span> ├® o seu maior gerador de receita. Ofere├ºa um cart├úo fidelidade para este servi├ºo.
                    </p>
                  </div>
                )}
                {margemReal < metas.margemDesejada && totalReceitas > 0 && (
                  <div className="p-5 sm:p-6 bg-amber-50 rounded-xl sm:rounded-2xl border border-amber-100 hover:shadow-md transition-all">
                    ...
                    <p className="text-xs sm:text-sm font-medium text-slate-700 leading-relaxed">
                      Sua margem de <span className="font-bold text-amber-600">{margemReal.toFixed(1)}%</span> est├í abaixo da meta. Avalie se os pre├ºos cobrem adequadamente todos os custos.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="animate-fadeIn pb-32 space-y-12 sm:space-y-16 lg:space-y-24 px-4 sm:px-6">
            <header className="flex items-center gap-4 sm:gap-6">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 sm:mt-2">{meses[mesAtual]} ÔÇó {anoAtual}</p>
              </div>
            </header>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
                <div className="p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2rem] bg-slate-900 text-white shadow-xl relative overflow-hidden group">
                  ...
                  <div className="flex flex-col gap-1 sm:gap-2 mb-6 sm:mb-8 min-w-0">
                    ...
                    <span className="text-[9px] sm:text-[11px] font-bold text-slate-400 uppercase truncate">M├¬s Anterior: {formatMoeda(totalReceitasAnterior)}</span>
                  </div>
                ...
                <div className="p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2rem] bg-white border border-slate-100 shadow-sm relative overflow-hidden group">
                  ...
                  <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 sm:mb-6">Lucro L├¡quido</p>
                  <div className="flex flex-col gap-1 sm:gap-2 mb-6 sm:mb-8 min-w-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16">
              <section className="bg-white p-4 sm:p-10 lg:p-14 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm">
                <h3 className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-8 sm:mb-12 pb-4 border-b border-slate-200">Evolu├º├úo Mensal (Faturamento)</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  ...
                  {analyticsData.historicoMensal.filter(h => h.faturamento > 0).length === 0 && (
                    <div className="col-span-full py-12 text-center text-slate-300 text-[10px] font-bold uppercase tracking-widest border-2 border-dashed border-slate-200 rounded-3xl">Sem dados hist├│ricos suficientes</div>
                  )}
                </div>
              </section>
              ...
              <section className="bg-white p-4 sm:p-10 lg:p-14 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm">
                ...
                <div className="space-y-6">
                  {[
                    ...
                    { label: 'Custos Vari├íveis', val: totalVariaveis, color: 'bg-rose-500', icon: <TrendingDown size={14} />, desc: 'Produtos, materiais, taxas...' }
                  ].map((g, idx) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 lg:gap-16">
              ...
              <div className="lg:col-span-4 flex flex-col gap-6 sm:gap-10">
                ...
                <div className="flex-1 bg-slate-900 p-6 sm:p-10 rounded-2xl sm:rounded-3xl flex flex-col items-center justify-center text-center shadow-xl">
                  <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 sm:mb-4 text-white/50">Efici├¬ncia</p>
            ...
            <div className="pt-4 px-4 sm:px-0">
              <button onClick={() => setView('dashboard')} className="w-full py-4 sm:py-5 bg-slate-900 text-white rounded-xl sm:rounded-2xl font-extrabold uppercase text-[10px] sm:text-xs tracking-widest shadow-lg hover:bg-slate-800 transition-all active:scale-[0.98]">Voltar para o Painel</button>
            </div>
          </div>
        ) : null}
        ...
        {[
          {
            isOpen: modalMetas, setOpen: setModalMetas, title: 'Definir Metas', content: (
              <div className="space-y-4 sm:space-y-6">
                ...
                <button onClick={() => setModalMetas(false)} className="w-full text-white py-3.5 sm:py-4 rounded-xl font-bold uppercase text-[10px] sm:text-xs tracking-widest mt-4 sm:mt-6" style={{ backgroundColor: appColor }}>Salvar Altera├º├Áes</button>
              </div>
            )
          },
          {
              ...
              <form onSubmit={(e) => {
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[9px] font-bold uppercase text-slate-400 ml-1 mb-1 block">M├¬s Alvo</label>
          ...
          {
            isOpen: modalNovoServico, setOpen: setModalNovoServico, title: 'Gerenciar Servi├ºos', content: (
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Servi├ºos Cadastrados</p>
                    ...
                    {servicos.map(s => (
                      <div key={s.id} className="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-xl hover:shadow-sm transition-all group">
                        ...
                        <div className="flex items-center gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => {
                            const novoNome = prompt('Novo nome do servi├ºo:', s.nome);
                          ...
                          <button onClick={() => {
                            if (confirm('Tem certeza que deseja remover este servi├ºo?')) {
          ...
          },
          {
            isOpen: modalClientes, setOpen: setModalClientes, title: 'Novo Cliente', content: (
              <form onSubmit={handleAddCliente} className="space-y-4 sm:space-y-6">
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Observa├º├Áes</label>
                  <textarea value={formCliente.observacoes} onChange={e => setFormCliente({ ...formCliente, observacoes: e.target.value })} className="w-full bg-slate-50 p-3.5 sm:p-4 rounded-xl font-bold border border-slate-200 outline-none text-sm min-h-[100px] resize-none" placeholder="Alergias, prefer├¬ncias, etc..." />
                </div>
          ...
          {
            isOpen: modalIA, setOpen: setModalIA, title: 'Consultoria Estrat├®gica IA', content: (
                    ...
                    <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 mb-6">
                      ...
                      <div className="text-slate-700 text-sm leading-relaxed prose prose-indigo prose-sm max-w-none font-medium font-outfit">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>
                          {aiResponse || "Carregando insights estrat├®gicos..."}
                        </ReactMarkdown>
                      </div>
                    </div>
                    <button
